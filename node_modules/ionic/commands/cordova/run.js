...
Just like with ${color_1.input('ionic cordova build')}, you can pass additional options to the Cordova CLI using the ${color_1.input('--')} separator. To pass additional options to the dev server, consider using ${color_1.input('ionic serve')} separately and using the ${color_1.input('--livereload-url')} option.
    ...
    async runServeDeploy(inputs, options) {
        ...
        if (!livereloadUrl) {
            ...
            const details = await serve_1.serve({ flags: this.env.flags, config: this.env.config, log: this.env.log, prompt: this.env.prompt, shell: this.env.shell, project: this.project }, inputs, utils_1.generateOptionsForCordovaBuild(metadata, inputs, options));
        ...
        if (options['native-run']) {
            ...
            await this.runNativeRun(native_run_1.createNativeRunArgs({ packagePath, platform, portForward }, options));
        }
        else {
    ...
    async runBuildDeploy(inputs, options) {
        ...
        if (options.build) {
            // TODO: use runner directly
            await build_1.build({ config: this.env.config, log: this.env.log, shell: this.env.shell, prompt: this.env.prompt, project: this.project }, inputs, utils_1.generateOptionsForCordovaBuild(metadata, inputs, options));
        }
        if (options['native-run']) {
            const [platform] = inputs;
            const packagePath = await project_1.getPackagePath(conf.getProjectInfo().name, platform, !options['device']);
    ...
    async runNativeRun(args) {
        ...
        await native_run_1.runNativeRun(this.env, args, { cwd: this.project.directory });
    }
}
