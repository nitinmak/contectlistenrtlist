    ...
    async run(inputs, options) {
        ...
        if (options['livereload']) {
            ...
            if (!livereloadUrl) {
                ...
                const details = await serve_1.serve({ flags: this.env.flags, config: this.env.config, log: this.env.log, prompt: this.env.prompt, shell: this.env.shell, project: this.project }, inputs, utils_1.generateOptionsForCapacitorBuild(inputs, options));
            ...
            if (options['build']) {
                // TODO: use runner directly
                await build_1.build({ config: this.env.config, log: this.env.log, shell: this.env.shell, prompt: this.env.prompt, project: this.project }, inputs, utils_1.generateOptionsForCapacitorBuild(inputs, options));
            }
        }
